...
function getProjectConfig(projectPath) {
    ...
    const baseWorktreePath = path_1.default.dirname(mainRepoPath);
    return {
        mainRepoPath,
        baseWorktreePath,
...
3. Verify with: echo "Hello, please respond with test works" | claude code --dangerously-skip-permissions`);
    ...
    const projectName = path_1.default.basename(config.mainRepoPath);
...
`);
    try {
        const orchestrator = new orchestrator_1.MultiAgentFeatureOrchestrator(config, featureName, projectName);
...
//# sourceMappingURL=run-feature.js.map