...
class FeedbackManager {
     ...
     */
    static generatePreviousFailureFeedback(reviewHistory, currentAttempt) {
        // Get the most recent round of rejected reviews 
        const recentRejectedReviews = this.getRecentRejectedReviews(reviewHistory, currentAttempt);
     ...
     */
    static getRecentRejectedReviews(reviewHistory, currentAttempt) {
        const rejectedReviews = reviewHistory.filter(review => review.result === 'REJECT');
        ...
        const recentRound = sortedReviews.filter(review => (mostRecentTimestamp - review.timestamp) <= timeWindow);
        return recentRound;
    }
    /**
     ...
     */
    static hasReworkRequiredFeedback(reviewResponse) {
        ...
        return (response.includes('must fix') ||
            response.includes('critical issues') ||
            response.includes('action items'));
    }
    static parseReviewResult(reviewResponse) {
        ...
        if (hasApprove && hasReject) {
            console.log(`⚠️ Both APPROVE and REJECT markers found - this is invalid, defaulting to REJECT`);
            return 'REJECT';
        }
    ...
    static parseReviewComments(reviewResponse) {
        const lines = reviewResponse.split('\n');
        const filteredLines = lines.filter(line => line.trim() && !line.includes('claude') && !line.includes('🤖'));
