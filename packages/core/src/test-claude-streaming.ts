...
async function testClaudeStreaming() {
  ...
  const simpleProcess = spawn(
    'bash',
    ['-c', 'echo "Say hello" | claude code --dangerously-skip-permissions'],
    {
...
Please make your response exactly like that with each step on its own line.`;
  ...
  const fileProcess = spawn(
    'bash',
    ['-c', `cat "${promptFile}" | claude code --dangerously-skip-permissions`],
    {
      stdio: ['pipe', 'pipe', 'pipe'],
  ...
  const largePromptFile =
    '/Users/nickschrock/git/stays-promotion-builder-v2/.codettea-solver-prompt.md';

  try {
    const largePromptContent = await fs.readFile(largePromptFile, 'utf-8');
    console.log(
      `📝 Found large prompt file (${largePromptContent.length} chars)`,
    );

    // Test with the actual large prompt
    const largeProcess = spawn(
      'bash',
      [
        '-c',
...
async function monitorProcess(
  ...
  return new Promise((resolve, reject) => {
    ...
    process.on('close', (code: number) => {
      ...
      console.log(
        `📊 Output: ${output.length} chars, Errors: ${errorOutput.length} chars`,
      );

      if (!hasOutput && code === 0) {
        console.log(
          `⚠️  Process completed successfully but no stdout received`,
        );
      }
