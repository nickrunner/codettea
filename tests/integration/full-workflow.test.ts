...
describe('Full Workflow Integration Tests', () => {
  ...
  describe('Template System Integration', () => {
    it('should load and customize all command templates', async () => {
      const templatesDir = path.join(__dirname, '../../src/commands');
    ...
    it('should customize templates with variables correctly', async () => {
      const orchestrator = new MultiAgentFeatureOrchestrator(testConfig, 'test-feature');
      
      const template = 'Feature: $FEATURE_NAME, Issue: $ISSUE_NUMBER, Path: $WORKTREE_PATH';
      const variables = {
  ...
  describe('Git Operations Integration', () => {
    it('should handle git worktree operations safely', async () => {
      ...
      const orchestrator = new MultiAgentFeatureOrchestrator(testConfig, 'test-git-ops');
      
      // This would test actual git operations in a controlled environment
      const mockWorktreePath = path.join(testConfig.baseWorktreePath, 'stays-test-git-ops');
  ...
  describe('GitHub CLI Integration', () => {
    ...
    it('should parse GitHub issue data correctly', async () => {
      ...
      const orchestrator = new MultiAgentFeatureOrchestrator(testConfig, 'test-feature');
      const dependencies = (orchestrator as any).parseDependencies(mockIssueData.body);
  ...
  describe('Claude Code CLI Integration', () => {
    ...
    it('should handle Claude Code execution timeouts', async () => {
      const orchestrator = new MultiAgentFeatureOrchestrator(testConfig, 'test-timeout');
      ...
      expect(async () => {
        ...
        await (orchestrator as any).callClaudeCodeAgent(mockPrompt, 'test', workingDir);
      }).not.toThrow();
    });
  });
  ...
  describe('Error Handling Integration', () => {
    ...
    it('should handle command failures gracefully', async () => {
      const orchestrator = new MultiAgentFeatureOrchestrator(testConfig, 'test-cmd-failures');
      
      // Test with a command that will fail
      try {
        await (orchestrator as any).callClaudeCodeAgent('test', 'test', '/non-existent');
      } catch (error) {
        expect(error).toBeDefined();
      }
    });
  ...
  async function setupTestRepository(): Promise<void> {
    try {
      ...
      await execAsync('git config user.name "Test User"', { cwd: testConfig.mainRepoPath });
      await execAsync('git config user.email "test@example.com"', { cwd: testConfig.mainRepoPath });
      ...
      await execAsync('git commit -m "Initial commit"', { cwd: testConfig.mainRepoPath });
