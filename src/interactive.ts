...
`);
    ...
    if (features.length === 0) {
      console.log('📭 No active feature branches found. Use "Start New Feature" to create one.');
      return;
    }
    ...
    const issueNumbers = choice.split(',')
...
`);
    ...
    if (sortedIssues.length > 0) {
      sortedIssues.forEach(issue => {
        ...
        const stepText = stepNum !== 999 ? `Step ${stepNum.toString().padStart(2, '0')}` : 'No Step';
        const stateIcon = issue.state === 'open' ? '🔴' : '✅';
        const progress = issue.inProgress ? ' 🚧' : '';
        const labels = issue.labels.length > 0 ? ` [${issue.labels.join(', ')}]` : '';
        
        console.log(`   ${stateIcon} #${issue.number.toString().padStart(2, ' ')} | ${stepText} | ${issue.title}${labels}${progress}`);
      });
    } else {
  ...
  private async workOnNextIssue(feature: FeatureStatus, sortedIssues: IssueStatus[]): Promise<void> {
    const openIssues = sortedIssues.filter(i => i.state === 'open');
  ...
  private async workOnSpecificIssue(feature: FeatureStatus, sortedIssues: IssueStatus[]): Promise<void> {
    const openIssues = sortedIssues.filter(i => i.state === 'open');
  ...
  private async handleAddIssues(featureName: string): Promise<void> {
    ...
    for (const issueNum of issues) {
      try {
        await execAsync(`gh issue edit ${issueNum} --add-label "${featureName}"`, {
...
`);
  ...
  private async createWorktree(): Promise<void> {
    ...
    try {
      ...
      try {
        const { stdout } = await execAsync(`git branch --list ${branchName}`, { cwd: this.config.mainRepoPath });
      ...
      if (branchExists) {
        ...
        await execAsync(`git checkout ${branchName}`, { cwd: this.config.mainRepoPath });
      } else {
        // Branch doesn't exist, create it
        await execAsync(`git checkout -b ${branchName}`, { cwd: this.config.mainRepoPath });
      }
...
`);
    ...
    try {
      ...
      const mergedMainBranches = mergedToMain.split('\n')
        .map(b => b.trim().replace('* ', ''))
        .filter(b => b && !['main', 'dev', 'master'].includes(b));
      
      const mergedDevBranches = mergedToDev.split('\n')
        .map(b => b.trim().replace('* ', ''))
        .filter(b => b && !['main', 'dev', 'master'].includes(b));

      const mergedBranches = [...new Set([...mergedMainBranches, ...mergedDevBranches])];
  ...
  private async deleteSpecificBranches(): Promise<void> {
    try {
      ...
      if (/^\d+(,\d+)*$/.test(selection.trim())) {
        ...
        branchesToDelete = selection.split(',').map(b => b.trim()).filter(b => b);
      }
...
`);
    ...
    if (!isNaN(parsedApprovals) && parsedApprovals >= 1 && parsedApprovals <= 5) {
  ...
  private async testConfiguration(): Promise<void> {
    ...
    try {
      ...
      const { stdout: _stdout } = await execAsync('git status', { cwd: this.config.mainRepoPath });
  ...
  private async getExistingFeatures(): Promise<FeatureStatus[]> {
    ...
    try {
      ...
      const { stdout: localBranches } = await execAsync('git branch', { cwd: this.config.mainRepoPath });
      
      // Combine and deduplicate branches
      const allBranches = [...remoteBranches.split('\n'), ...localBranches.split('\n')];
  ...
  private async showWorktreeStatus(worktreePath: string): Promise<void> {
    ...
    try {
      const { stdout: status } = await execAsync('git status --short', { cwd: worktreePath });
      ...
      // Show recent commits
      const { stdout: log } = await execAsync('git log --oneline -5', { cwd: worktreePath });
      console.log('\n📚 Recent Commits:');
      console.log(log);
